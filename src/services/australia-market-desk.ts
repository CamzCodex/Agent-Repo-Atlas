...
export interface AustraliaQuoteGroupStatus {
  ...
  dataOffline: boolean;
  /** State of the newest fetch invocation, even when older data is retained. */
...
export interface AustraliaMarketDeskOptions {
  ...
  /** Explicit states override weakly associated array metadata. */
...
function buildQuoteObservation(
  ...
  const provenance = assessFinanceObservationProvenance({
    ...
    notes: quote
      ? [
          ...
          `Latest fetch-invocation mode: ${groupStatus.latestAttemptMode}${groupStatus.latestAttemptOffline ? ' (offline)' : ''}.`,
  ...
  return {
    ...
    latestAttemptMode: groupStatus.latestAttemptMode,
    latestAttemptOffline: groupStatus.latestAttemptOffline,
    provenance,
...
export function buildAustraliaMarketDeskSnapshot(
  ...
  if (marketGroupStatus.latestAttemptMode === 'unavailable' && markets.some((entry) => entry.quote !== null)) {
    ...
    marketGroupStatus.dataMode === 'cached'
    && marketGroupStatus.latestAttemptMode === 'live'
    && markets.some((entry) => entry.quote !== null)
  ) {
    warnings.push('Latest Australian equity refresh returned no usable observations; displaying the last usable observations.');
  }
  if (resourceGroupStatus.latestAttemptMode === 'unavailable' && resources.some((entry) => entry.quote !== null)) {
    ...
    resourceGroupStatus.dataMode === 'cached'
    && resourceGroupStatus.latestAttemptMode === 'live'
    && resources.some((entry) => entry.quote !== null)
  ) {
    warnings.push('Latest AUD/resource refresh returned no usable observations; displaying the last usable observations.');
  }
  if (
